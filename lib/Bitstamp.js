...
function RestClient(config) {
    ...
    this.order = function(param){
        ...
        var amount = this.convertVolume(param.market, param.amount);
        var price = this.convertPrice(param.market, param.price, param.type);
        log.info("order: amount: %s, price: %s", amount, price);
        ops(amount, price, function(err, result){
            debug("order result")
            if(err){
