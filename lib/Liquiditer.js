...
function Liquiditer(config) {
    ...
    function cancelOrder(side, removed){
        if(removed.length === 0){
            return;
        }
        debug("cancelOrder #%s %d", side, removed.length);
    ...
    function sendOrders(bidasks, type) {
        ...
        debug("sendOrders %s %d, ", type, bidasks.length, bidasks);
        var deferred = Q.defer();
    ...
    function filterAsks(asksOrig){
        ...
        //debug('filterAsks asks     ', asksOrig);
    ...
    function filterBids(bidsOrig){
        ...
        //debug('onOrderBookDiff b4 bids ', bidsOrig);
    ...
    function onOrderBook(me, orderBook){
        ...
        .then(function(){
            ...
            return Q.all([cancelOrder('bid', diff.diffBids.removed),cancelOrder('ask', diff.diffAsks.removed)])
        })
        .then(function(){
    ...
    function onActivity(activity){
        log.info("onActivity: ", JSON.stringify(activity));

        if(activity.type === 'FillOrder'){
            onFillOrder(activity.details);
        }
        if(activity.type === 'FillOrderPartial'){
            onFillOrder(activity.details);
        }
    }
    
    function onFillOrder(order){
        log.info("onFillOrder: ", JSON.stringify(order));
        var param = {
                market: order.market,
                orderType: "market",
                amount: order.filled
        }
        var bnPrice = BigNumber(order.price);
        if(order.type === 'bid'){
            param.type = "ask";
            var bnFeeBid = BigNumber(_provider.config().fee_bid);
            param.price = bnPrice.times(100).dividedBy(bnFeeBid.plus(100)).toString();
        } else {
            param.type = "bid";
            var bnFeeAsk = BigNumber(_provider.config().fee_ask);
            param.price = bnPrice.times(100).dividedBy(bnFeeAsk.plus(100)).toString();
        }
        log.info("onFillOrder ", JSON.stringify(param));
        
        if(!config.trade){
            log.info("onFillOrder no trading");
            return;
        }
        
        _provider.order(param)
        .then(function(result){
            log.info("onFillOrder ", JSON.stringify(result));
        })
        .fail(function(err){
            log.error("onFillOrder ", err);
            //onError();
        })
    }
    ...
    function orderBookInit(){
        ...
        configOrderBook.bidMinPrice = marketInfo.bidminprice;
        configOrderBook.askMaxPrice = marketInfo.askmaxprice;
    ...
    this.start = function() {
        ...
        .fail(function(error){
            log.error("started with error: ", JSON.stringify(error));
            me.emit('error', error)
