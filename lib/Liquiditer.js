...
function Liquiditer(config) {
    ...
    function onFillOrder(order){
        ...
        if(order.type === 'bid'){
            param.type = "ask";
            param.price = num(order.price).mul(100).div(num(100).add(_provider.config().fee_bid)).toString()
        } else {
            param.type = "bid";
            param.price = num(order.price).mul(100).div(num(100).add(_provider.config().fee_ask)).toString()
        }
